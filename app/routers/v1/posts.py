...
async def get_search_posts(
    ...
    posts = await post_service.get_search_posts(q, offset, limit, db, sort, order)
...
async def delete_like(post_id: UUID, user_id: UUID, db: Session = Depends(get_db)):
    try:
        post_service.delete_like(post_id, user_id, db)
        db.commit()
...
async def delete_image(post_id: UUID, image_name: str, db: Session = Depends(get_db)):
    try:
        post_service.delete_image(post_id, image_name, db)
        db.commit()
...
async def delete_post(post_id: UUID, db: Session = Depends(get_db)):
    try:
        post_service.delete_post(post_id, db)
        db.commit()
